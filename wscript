...
from taxon.adcasic import *
from taxon.feasic import *
from taxon.femb import *


def build(bld):

    here = globals()             # to lookup taxa functions
    taxa_dat = defaultdict(list) # hold metadata keyed by taxon name

    for taxon in taxa:
        print ("Building for %s" % taxon)
        seeder = here["seed_"+taxon]
        builder = here["build_"+taxon]
        for count, seed in enumerate(seeder(bld, **taxa_dat)):
            dat = builder(bld, seed, **taxa_dat)
            if not dat:
                continue
